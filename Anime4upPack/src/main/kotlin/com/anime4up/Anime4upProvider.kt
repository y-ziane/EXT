        ...
        if(data.contains("anime4up")) {
            ...
            if (moshahdaID.isNotEmpty()) {
                mapOf(
                    ...
                    "360"      to "download_n",
                    "240"      to "download_l"
                ).apmap { (qualityLabel, qualityCode) ->
                    callback.invoke(
                        newExtractorLink(
                            this.name,
                            "${this.name} Moshahda",
                            "https://moshahda.net/$moshahdaID.html?${qualityCode}",
                            ExtractorLinkType.VIDEO          // <— 4th positional param is the type
                        ) {
                            // this: ExtractorLink
                            referer = "https://moshahda.net"  // set referer here
                            quality = qualityLabel.toIntOrNull() ?: 1080
                        }
                    )
                }
            }
